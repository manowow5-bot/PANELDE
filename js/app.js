(function () {
  ...
  function initLogin() {
    ...
    if (!isCloudAuthEnabled()) {
      setMessage("loginMessage", "Configura CLOUD_AUTH_API_KEY para usar cuentas en cualquier dispositivo.", "error");
      return;
    }
    ...
    form.addEventListener("submit", async function (event) {
      ...
      const localUser = await ensureLocalUserFromEmail(email, password);
  ...
  function initRegister() {
    ...
    if (!isCloudAuthEnabled()) {
      setMessage("registerMessage", "Configura CLOUD_AUTH_API_KEY para registrar usuarios en cualquier dispositivo.", "error");
      return;
    }
    ...
    form.addEventListener("submit", async function (event) {
      ...
      await ensureLocalUserFromEmail(email, password, fullName, whatsapp);
  ...
  function initRecover() {
    ...
    if (!isCloudAuthEnabled()) {
      setMessage("recoverMessage", "Configura CLOUD_AUTH_API_KEY para recuperación multi-dispositivo.", "error");
      return;
    }
